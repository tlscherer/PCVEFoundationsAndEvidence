...
This .rmd creates all the figures used in the report "PCVE Foundations and Evidence".
...
Download the datasheet, load it, and prepare the data 
```{r warning=FALSE}
target_file <- paste0(here::here(),"/data/literature_datasheet.xlsx")

downloaded_file <- googledrive::drive_download(file="https://docs.google.com/spreadsheets/d/1QCnLUwvHdO8qGGpwNYCdU_2eycPX1PkUnuxRS8RGkbs/edit#gid=1007627490", path=target_file, type = "xlsx", overwrite=T)
...
```{r}
youth_types <- datasheet_youth %>% dplyr::count(document_type, sort=TRUE) %>% 
  dplyr::rename(Youth = n)
  
gender_types <- datasheet_gender %>% dplyr::count(document_type, sort=TRUE) %>% 
  dplyr::rename(Gender = n)

religion_types <- datasheet_religion %>% dplyr::count(document_type, sort=TRUE) %>% 
  dplyr::rename(Religion = n)

community_types <- datasheet_community %>% dplyr::count(document_type, sort=TRUE) %>% 
...
```{r}
#Youth
youth_primary <- datasheet_youth %>% dplyr::filter(document_type %in% c("research", "program evaluation"))
...
#Gender
gender_primary <- datasheet_gender %>% dplyr::filter(document_type %in% c("research", "program evaluation"))
...
#Religion
religion_primary <- datasheet_religion %>% dplyr::filter(document_type %in% c("research", "program evaluation"))
...
#community
community_primary <- datasheet_community %>% dplyr::filter(document_type %in% c("research", "program evaluation"))
...
all_type_timecount <- datasheet_all %>% dplyr::count(year, document_type) %>% 
  dplyr::full_join(tidyr::expand_grid(document_type = unique(datasheet_all$document_type), year = unique(datasheet_all$year))) %>% 
  dplyr::mutate(year=as.numeric(year)) %>% dplyr::filter(!(is.na(year)), !(is.na(document_type))) %>% dplyr::mutate(n = ifelse(is.na(n), 0, n))

ggplot(all_type_timecount, aes(x=year, y=n, fill=document_type)) + 
  geom_area() + 
...
youth_type_timecount <- datasheet_youth %>% dplyr::count(year, document_type) %>% 
  dplyr::full_join(tidyr::expand_grid(document_type = unique(datasheet_youth$document_type), year = unique(datasheet_youth$year))) %>% 
  dplyr::mutate(year=as.numeric(year)) %>% dplyr::filter(!(is.na(year)), !(is.na(document_type))) %>% dplyr::mutate(n = ifelse(is.na(n), 0, n))

ggplot(youth_type_timecount, aes(x=year, y=n, fill=document_type)) + 
    geom_area() 
...
gender_type_timecount <- datasheet_gender %>% dplyr::count(year, document_type) %>% 
  dplyr::full_join(tidyr::expand_grid(document_type = unique(datasheet_gender$document_type), year = unique(datasheet_gender$year))) %>% 
  dplyr::mutate(year=as.numeric(year)) %>% dplyr::filter(!(is.na(year)), !(is.na(document_type))) %>% dplyr::mutate(n = ifelse(is.na(n), 0, n))

ggplot(gender_type_timecount, aes(x=year, y=n, fill=document_type)) + 
    geom_area()
